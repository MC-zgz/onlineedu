      ...
      <li v-for="chapter in chapterVideoList" :key="chapter.id">
        <p>
          ...
          <span class="acts">
            ...
            <el-button style type="text" @click="openUpdateChapterInfo(chapter.id)">编辑</el-button>
        ...
        <ul class="chanpterList videoList">
          <li v-for="video in chapter.children" :key="video.id">
            <p>
              {{ video.title }}
              <span class="acts">
                <el-button type="text" @click="openUpdateVideoInfo(video.id)">编辑</el-button>
      ...
      <el-form :model="eduVideo" label-width="120px">
        ...
        <el-form-item label="上传视频">
          <el-upload :on-success="handleVodUploadSuccess" :on-remove="handleVodRemove" :before-remove="beforeVodRemove"
            :on-exceed="handleUploadExceed" :file-list="fileList" :action="BASE_API+'/vodservice/vod/uploadVideo'"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //打开章节信息页面
      openUpdateChapterInfo(chapterId) {
        chapter.getChapterById(chapterId)
          .then(res => {
            this.eduChapter = res.data.eduChapter;
            this.dialogChapterFormVisible = true;
          })
      ...
      updateChapterInfo() {
        //1调用修改章节接口
        chapter.updChapter(this.eduChapter)
          .then(res => {
            this.$message({
              type: "success",
              message: "修改成功!"
      ...
      delChapterInfo(chapterId) {
        this.$confirm("此操作将永久删除该章节, 是否继续?", "提示", {
          ...
          chapter.delChapter(chapterId).then(res => {
            //删除成功
            console.log("删除成功");
            this.$message({
              type: "success",
      ...
      openAddVideo(chapterId) {
        ...
        // console.log("chapterId:" + this.eduVideo.chapterId);
      ...
      openUpdateVideoInfo(videoId) {
        video.getVideoById(videoId)
          .then(res => {
            // console.log(this.eduVideo.chapterId);
            this.eduVideo = res.data.eduVideo;
            this.dialogVideoFormVisible = true;
          })
      ...
      delVideoInfo(videoId) {
        this.$confirm("此操作将永久删除该小节, 是否继续?", "提示", {
          ...
          video.delVideo(videoId).then(res => {
            //删除成功
            console.log("删除成功");
            this.$message({
              type: "success",
      ...
      handleVodUploadSuccess(res, file, fileList) {
        this.eduVideo.videoSourceId = res.data.videoId;
      },
      //视图上传多于一个视频
